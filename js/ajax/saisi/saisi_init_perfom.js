$(document).ready(function() {
    ...
    btnControle.on('click',function(e)
    {
        ...
        else
        {
            ...
            var traitementNumActeVide =  $.post('../../proccess/ajax/saisi/controle_num_acte_vide.php',   // url
                { myData: JSON.stringify(data1) }, // data to be submit
                function(data, status, jqXHR) 
                {
                    var result = JSON.parse(data);                               
                    ...
                    if(result[0] == "success")
                    {
                        // success callback
                        // display result    
                        $("#liste-indic li:eq(1)").html("Traitement Numéro Acte Vide : (" + result[1].length + ") <i class='fas fa-check text-success' style='margin-left:5px;font-size:20px;'></i>");
                        $("#liste-indic li:eq(1)").fadeIn(1000);
                        console.log('success : ' + result[1]);
                        console.log(result[1]);

                        // Lancement Num Acte ne correspond pas à imagepath
                        traitementNumActeDiffImagepath(); 
                    }
                    else
                    {
                        console.log('message error : ' + result);
            ...
            var traitementNumActeDiffImagepath =  $.post('../../proccess/ajax/saisi/controle_num_acte_diff_imagepath.php',   // url
                { myData: JSON.stringify(data1) }, // data to be submit
                function(data, status, jqXHR) 
                {
                    var result = JSON.parse(data);                               
                    ...
                    if(result[0] == "success")
                    {
                        // success callback
                        // display result    
                        $("#liste-indic li:eq(2)").html("Traitement Num Acte # imagepath : (" + result[1].length + ") <i class='fas fa-check text-success' style='margin-left:5px;font-size:20px;'></i>");
                        $("#liste-indic li:eq(2)").fadeIn(1000);
                        console.log('success : ' + result[1]);
                        console.log(result[1]);

                        // Lancement Traitement Image saisit en double
                        traitementImagepathDouble();
                    }
                    else
                    {
                        console.log('message error : ' + result);
            ...
            var traitementNumACteDouble =  $.post('../../proccess/ajax/saisi/controle_num_acte_double.php',   // url
                { myData: JSON.stringify(data1) }, // data to be submit
                function(data, status, jqXHR) 
                {
                    var result = JSON.parse(data);                               
                    ...
                    if(result[0] == "success")
                    {
                        // success callback
                        // display result    
                        $("#liste-indic li:eq(3)").html("Traitement Num_Acte en double : (" + result[1].length + ") <i class='fas fa-check text-success' style='margin-left:5px;font-size:20px;'></i>");
            ...
            var traitementImagepathDouble =  $.post('../../proccess/ajax/saisi/controle_image_double.php',   // url
                { myData: JSON.stringify(data1) }, // data to be submit
                function(data, status, jqXHR) 
                {
                    var result = JSON.parse(data);                               
                    ...
                    if(result[0] == "success")
                    {
                        // success callback
                        // display result    
                        $("#liste-indic li:eq(4)").html("Traitement Image saisit en double : (" + result[1].length + ") <i class='fas fa-check text-success' style='margin-left:5px;font-size:20px;'></i>");
