...
try {
    ...
    foreach ($liste_lots as $lot) {
        $qry = $bdd->prepare("SELECT af.id_lot,a.num_acte,a.id_acte,a.imagepath
                                ...
                                union
                                select af.id_lot,mde.num_acte,id_evenement,mde.imagepath
        ...
        foreach ($listImageLot as $ImageLot) {
            ...
            $nbImageLot += count(array_values(array_filter($nbImageSlipt, function ($e) {
                return !empty(trim($e));
            })));
            $doubleImage += ($nbImageLot > 1) ? count(array_values(array_filter($nbImageSlipt, function ($e) {
                return !empty(trim($e));
            }))) - 1 : 0;
        }
        ...
        $currentLot = array_values(array_filter($liste_extraDb, function ($e) use ($lot_parse) {
            return $e->lot . '' == $lot_parse . '';
        }));
        $nbImageLotStats = 0;
        ...
        if (!empty($repChemin) && is_dir($repChemin)) {
            ...
            $nbImageDirectory = array_values(array_filter($Dirfiles, function ($e) {
                return preg_match("/.jpg$/", $e);
            }));
            if ((count($nbImageDirectory) - 2) != $nbImageLot  || (count($nbImageDirectory) - 2)  != $nbImageLotStats || $nbImageLot != $nbImageLotStats) {
                $nbLot++;
                $liste_el_select[] = [
                    "id_lot" => $lot->id_lot, "nb_image_inv" => $nbImageLotStats, "nb_image_db" => $nbImageLot, "nb_image_repos" =>  count($nbImageDirectory), "chemin_lot" =>  $repChemin, "observation" => ''
                ];
            }
        } else {
            $nbLot++;
            $liste_el_select[] = [
                "id_lot" => $lot->id_lot, "nb_image_inv" => $nbImageLotStats, "nb_image_db" => $nbImageLot, "nb_image_repos" =>  "0", "chemin_lot" =>  $repChemin, "observation" => 'REPERTOIRE INTRROUVABLE'
            ];
        }
    }
