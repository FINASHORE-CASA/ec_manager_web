...
try {
    ...
    $qry = $bdd->prepare("  SELECT af.id_lot,iv.id_type_registre,iv.annee,iv.numero_tome,iv.id_bureau,(CASE WHEN iv.indice_num_tome is null THEN 0 ELSE iv.indice_num_tome END) as indice_num_tome
                                ...
                                ,iv.nbre_acte_naissance,iv.nbre_acte_deces,iv.nbre_acte_mariage,iv.nbre_acte_divorce
                                ,iv.nbre_acte_naissance + iv.nbre_acte_deces + iv.nbre_acte_mariage + iv.nbre_acte_divorce as total,iv.id_inventaire
    ...
    foreach ($liste_db_inventaires as $db_inv) {
        ...
        if (count($find) > 0) {
            ...
            if (
                $db_inv->indice_num_tome != $find[0]->indice || $db_inv->numero_tome != $find[0]->tome || $db_inv->annee != $find[0]->annee_g
                || $db_inv->annee_h != $find[0]->annee_h || $db_inv->nbre_acte_naissance != $find[0]->naissance || $db_inv->nbre_acte_deces != $find[0]->deces
                || $db_inv->nbre_acte_mariage != $find[0]->mariage || $db_inv->nbre_acte_divorce != $find[0]->divorce
                || $db_inv->total != $find[0]->total
            ) {
                $liste_el_select[] = [
                    "id_inventaire" => $db_inv->id_inventaire, "id_lot" => $db_inv->id_lot, "indice_db" => $db_inv->indice_num_tome, "indice_inv" => $find[0]->indice, "tome_db" => $db_inv->numero_tome, "tome_inv" => $find[0]->tome, "annee_g_db" => $db_inv->annee, "annee_g_inv" => $find[0]->annee_g, "annee_h_db" => $db_inv->annee_h, "annee_h_inv" => $find[0]->annee_h, "naissance_db" => $db_inv->nbre_acte_naissance, "naissance_inv" => $find[0]->naissance, "deces_db" => $db_inv->nbre_acte_deces,
                    "deces_inv" => $find[0]->deces, "mariage_db" => $db_inv->nbre_acte_mariage, "mariage_inv" => $find[0]->mariage, "divorce_db" =>  $db_inv->nbre_acte_divorce, "divorce_inv" => $find[0]->divorce,
                    "acte_db" => $db_inv->total, "acte_inv" => $find[0]->total
                ];

                // Correction automatique
                $qry = $bdd->prepare("UPDATE inventaire_bec set annee = :annee_g, annee_h = :annee_h,nbre_acte_naissance = :nb_naiss, nbre_acte_deces = :nb_deces,nbre_acte_mariage = :nb_mariage,nbre_acte_divorce = :nb_divorce where id_inventaire = :id_inventaire;");
            ...
            $liste_el_select[] = [
                "id_inventaire" => $db_inv->id_inventaire, "id_lot" => $db_inv->id_lot, "indice_db" => $db_inv->indice_num_tome, "indice_inv" => "ND", "tome_db" => $db_inv->numero_tome, "tome_inv" => "ND", "annee_g_db" => $db_inv->annee, "annee_g_inv" => "ND", "annee_h_db" => $db_inv->annee_h, "annee_h_inv" => "ND", "naissance_db" => $db_inv->nbre_acte_naissance, "naissance_inv" => "ND", "deces_db" => $db_inv->nbre_acte_deces,
                "deces_inv" => "ND", "mariage_db" => $db_tr->nbre_acte_mariage, "mariage_inv" => "ND", "divorce_db" =>  $db_tr->nbre_acte_divorce, "divorce_inv" => "ND",
                "acte_db" => $db_inv->total, "acte_inv" => "ND"
            ];
        }
    }
