<?php
    ...
    foreach($registres_index as $registre)
    {
        ...
        $qry = $bdd->prepare("SELECT ag.*
                              ...
                              WHERE tr.TableSelect = 'Registres' AND tr.TypeTraitement = 7 AND tr.TableID = ?");
        ...
        WHERE RegistreID = {$registre->RegistreID} AND StatutActuel = 3");
        $qry->execute();
        ...
        {
            $percent_index = number_format($nb_imagesTermineRegistre[0] / $nb_imagesRegistre[0] * 100,2);
        }
        else
        {
            $percent_index = number_format(0,2);
        }
        ...
        echo "  
            <tr>
                ...
                <td> {$registre->Type} </td>
                <td> <strong> {$percent_index} % </strong></td>
                <td> ".date('d/m/Y à H:i ',strtotime($registre->DateModif))." </td>
                <td> 
                    <a class=\"btn btn-primary btn-icon-split\" 
                    href=\"details_registre.php?id_registre={$registre->RegistreID}&page_sender=etatIndex\">
        ...
        if($percent_index < 25.00)
        {
            $Data_color[] = '#4e73df'; // blue
        }
        else if($percent_index < 50.00)
        {
            $Data_color[] = '#f6c23e'; // yellow
        }
        else if($percent_index < 75.00)
        {
            $Data_color[] = '#fd7e14'; // orange
        }
        else if($percent_index < 100.00)
        {   
