<?php
    ...
    $qry = $bdd->prepare("SELECT * FROM {$base_de_donnees}.dbo.Registres rg
                          ...
                          WHERE rg.StatutActuel = 6 AND cr.StatutCorrection = 1 
                          AND cr.PhaseCorrection = 3 AND cr.ImageID is null 
                          AND cr.SequenceID is null");
    $qry->execute();
    $registres_cr_ph3 = $qry->fetchAll(PDO::FETCH_OBJ);
    $qry->closeCursor();
    $maxY = 100;
    $maxX = 0;

    foreach($registres_cr_ph3 as $registre)
    {
        ...
        {
            $percent_cr_ph3 = number_format($nb_imagesTermineRegistre[0] / $nb_imagesRegistre[0] * 100,2);            
        }
        else
        {
            $percent_cr_ph3 = 0.00;
        ...
        echo "  
            <tr>
                ...
                <td> <strong> {$percent_cr_ph3} % </strong></td>
        ...
        if($percent_cr_ph3 < 25.00)
        {
            $Data_color[] = '#4e73df'; // blue
        }
        else if($percent_cr_ph3 < 50.00)
        {
            $Data_color[] = '#f6c23e'; // yellow
        }
        else if($percent_cr_ph3 < 75.00)
        {
            $Data_color[] = '#fd7e14'; // orange
        }
        else if($percent_cr_ph3 < 100.00)
        {   
