...
try {
    ...
    $posLetter2 = 0;
    while (count($ExcelLetters) <= count((array)$formData[0])) {
        $IdLetter = (count($ExcelLetters) % 26 == 0) ?  (count($ExcelLetters) / 26) + 1 : ceil(count($ExcelLetters) / 26);
        $posLetter2 = (count($ExcelLetters) % 26 == 0) ? 0 : $posLetter2 + 1;
        $ExcelLetters[] = $ExcelLetter[($IdLetter - 2)] . $ExcelLetter[$posLetter2];
    }

    // Définition des colonnes 
    $i = 0;
    foreach ($formData[0] as $key => $value) {
        $sheet->setCellValue($ExcelLetters[$i] . '1', strtoupper($key));
        $i++;
    }

    foreach ($ExcelLetters as $value) {
    ...
    $sheet->getStyle("A1:{$ExcelLetters[($i - 1)]}1")->applyFromArray($styleHeader);
    ...
    foreach ($formData as $element) {
        ...
        foreach ($element as $key => $value) {
            $sheet->setCellValue($ExcelLetters[$c] . $row, $value);
            $c++;
        }
        $row++;
    }
    ...
    $file_name = 'CACHE_TEMP_FILE_' . $date_gen_xlsx . '.xlsx';
