...
try {
    ...
    foreach ($IdLotASup as $i => $lot) {
        $listLot .= (count($IdLotASup) > $i + 1) ? $lot->id_lot . ", " : $lot->id_lot;
    }
    ...
    if ($listLot != "") {
        ...
        $nbAff12 = $bdd->exec("DELETE from lot where id_lot in ($listLot)");
        ...
        throw new Exception("Lots inexistants dans la base de données", 1);
    }
