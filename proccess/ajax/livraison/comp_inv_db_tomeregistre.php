<?php
    ...
    try 
    {
        ...
        $qry = $bdd->prepare("  SELECT af.id_lot,tr.num_tome,(CASE WHEN tr.indice_num_tome is null THEN 0 ELSE  tr.indice_num_tome END) 
                                ...
                                ,tr.nbre_acte_naissance + tr.nbre_acte_deces as total,tr.id_tome_registre
        ...
        foreach ($liste_db_tomeregistre as $db_tr) 
        {
           ...
           if(count($find) > 0)
           {
               ...
               if($db_tr->indice_num_tome != $find[0]->indice || $db_tr->num_tome != $find[0]->tome || $db_tr->annee_registre_greg != $find[0]->annee_g
                || $db_tr->annee_registre_hegire != $find[0]->annee_h || $db_tr->nbre_acte_naissance != $find[0]->naissance || $db_tr->nbre_acte_deces != $find[0]->deces
                || $db_tr->total != $find[0]->total)
               {
                    $liste_el_select[] = ["id_tome_registre" => $db_tr->id_tome_registre,"id_lot" => $db_tr->id_lot,"indice_db" => $db_tr->indice_num_tome,"indice_inv" => $find[0]->indice,"tome_db" => $db_tr->num_tome,"tome_inv" => $find[0]->tome
                                          ,"annee_g_db" => $db_tr->annee_registre_greg,"annee_g_inv" => $find[0]->annee_g,"annee_h_db" => $db_tr->annee_registre_hegire, "annee_h_inv" => $find[0]->annee_h
                                          ,"naissance_db" => $db_tr->nbre_acte_naissance,"naissance_inv" => $find[0]->naissance,"deces_db" => $db_tr->nbre_acte_deces,
                                          "deces_inv" => $find[0]->deces,"acte_db" => $db_tr->total, "acte_inv" => $find[0]->total];

                    // Correction automatique
                    $qry = $bdd->prepare("UPDATE tomeregistre set nbre_acte_naissance = :nb_naiss,nbre_acte_deces = :nb_deces where id_tome_registre = :tome_registre;");
            ...
            {
                $liste_el_select[] = ["id_tome_registre" => $db_tr->id_tome_registre,"id_lot" => $db_tr->id_lot,"indice_db" => $db_tr->indice_num_tome,"indice_inv" => "ND","tome_db" => $db_tr->num_tome,"tome_inv" => "ND"
                                    ,"annee_g_db" => $db_tr->annee_registre_greg,"annee_g_inv" => "ND","annee_h_db" => $db_tr->annee_registre_hegire, "annee_h_inv" => "ND"
                                    ,"naissance_db" => $db_tr->nbre_acte_naissance,"naissance_inv" => "ND","deces_db" => $db_tr->nbre_acte_deces,
                                    "deces_inv" => "ND","acte_db" => $db_tr->total, "acte_inv" => "ND"];
            }
        }
