        ...
        {            
            ...
            if(str_contains($num_acte,"_"))
            {
                ...
                if (count($num_acte_split) == 2)
                {
                    ...
                    $liste_el_select[] = ["id_lot" => $lot->id_lot,"id_acte" => $lot->id_acte,"old_num_acte" => $lot->num_acte,"num_acte" => ($num_acte_split[0]."/".$num_acte_split[1])
                    ,"imagepath" => $lot->imagepath ,"observation" => "CORRIGE"];
                }
                else
                {
                    $liste_el_select[] = ["id_lot" => $lot->id_lot,"id_acte" => $lot->id_acte,"old_num_acte" => $lot->num_acte,"num_acte" => $lot->num_acte
                    ,"imagepath" => $lot->imagepath ,"observation" => "NON CORRIGE"];
                }
            }
            else
            {
                ...
                $liste_el_select[] = ["id_lot" => $lot->id_lot,"id_acte" => $lot->id_acte,"old_num_acte" => $lot->num_acte,"num_acte" => $num_acte
                ,"imagepath" => $lot->imagepath ,"observation" => "CORRIGE"];
            }
        }
